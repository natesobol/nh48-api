(() => {
  ...
  const hasDevBirdDataSource = Object.prototype.hasOwnProperty.call(window, '__NH_BIRD_DEV_DATA__');
  ...
  function emitBirdCatalogFilterChange(trigger) {
    trackAnalytics('bird_catalog_filter_change', {
      trigger,
  ...
  function normalizeBirdRecord(raw, index = 0) {
    ...
    const id = normalizeToken(slugSource);
    if (!id) return null;

    const tags = asStringList(raw.tags || raw.labels || raw.keywords || raw.badges);
    const images = []
    ...
    return {
      id,
      common: common || `Bird ${index + 1}`,
      latin,
      type: String(raw.type || raw.group || raw.category || raw.familyGroup || '').trim(),
      habitat: String(raw.habitat || raw.primaryHabitat || raw.habitatNotes || '').trim(),
      bloom: String(raw.season || raw.migrationWindow || raw.bestSeason || raw.peakSeason || '').trim(),
      elevation: String(raw.elevation || raw.elevationRange || '').trim(),
      teaser: String(raw.teaser || raw.summary || raw.overview || '').trim(),
      desc: String(raw.desc || raw.description || raw.notes || '').trim(),
      tags,
      imgs: uniqueImages,
  ...
  function resolveBirdDataSource() {
  ...
  function resolveBirdIndexSource() {
    const payload = window.__NH_BIRD_DEV_INDEX__;
    if (!payload) return { plants: {}, facets: {} };
  ...
  function localizeCatalogAnchors() {
    if (!IS_FRENCH_ROUTE) return;
    document.querySelectorAll('a[href^="/bird-catalog"]').forEach((anchor) => {
      const href = String(anchor.getAttribute('href') || '');
      if (!href || href.startsWith('/fr/bird-catalog')) return;
      anchor.setAttribute('href', href.replace(/^\/bird-catalog/, '/fr/bird-catalog'));
    });
  }

  function updateBrowserUrl(mode = 'push') {
    const relativeUrl = buildCatalogRelativeUrl(state, { includeDefaults: false });
    const historyState = { birdCatalogState: { ...state } };
    if (mode === 'replace') {
  ...
  function updateBirdItemList(filteredPlants) {
    const birdListScript = document.querySelector('script.bird-list');
    if (!birdListScript) return;
    try {
      const birdItemList = JSON.parse(birdListScript.textContent || '{}');
      birdItemList.itemListElement = filteredPlants.map((plant, idx) => ({
        '@type': 'ListItem',
        position: idx + 1,
        name: plant.common,
        url: `${SITE_ORIGIN}${DETAIL_PATH_PREFIX}/${encodeURIComponent(plant.id)}`,
      }));
      birdListScript.textContent = JSON.stringify(birdItemList);
    } catch (error) {
      console.error('Failed to update bird ItemList structured data', error);
    }
  }
  ...
  function cardHTML(plant) {
    ...
    const teaserText = String(plant.teaser || plant.desc || '').slice(0, 220);
    ...
    return `
      <div class="thumb">
        ...
        <img src="${LQ_PLACEHOLDER}" data-src="${escapeHtml(src)}"
             alt="${escapeHtml(plant.common)} bird catalog card image"
      ...
      <div class="body">
        <div class="card-title-row"><h3>${escapeHtml(plant.common)}</h3></div>
        <div class="latin">${escapeHtml(plant.latin || '')}</div>
        <div class="meta">
          ${plant.type ? `<div class="meta-row"><span class="meta-label">Type</span><span class="meta-value">${escapeHtml(plant.type)}</span></div>` : ''}
          ${plant.habitat ? `<div class="meta-row"><span class="meta-label">Habitat</span><span class="meta-value">${escapeHtml(plant.habitat)}</span></div>` : ''}
          ${plant.elevation ? `<div class="meta-row"><span class="meta-label">Elevation</span><span class="meta-value">${escapeHtml(plant.elevation)}</span></div>` : ''}
          ${plant.bloom ? `<div class="meta-row"><span class="meta-label">Season</span><span class="meta-value">${escapeHtml(plant.bloom)}</span></div>` : ''}
          ${(visibleTags.length || overflowTagMarkup) ? `<div class="meta-row"><span class="meta-label">Tags</span><span class="meta-value"><span class="meta-tags">${tagsMarkup}${overflowTagMarkup}</span></span></div>` : ''}
        </div>
        ${plant.teaser ? `<div class="teaser-block"><span class="teaser-label">Overview</span><p class="teaser">${escapeHtml(plant.teaser)}</p></div>` : ''}
        <div class="more">></div>
      </div>`;
  }
  ...
  function updateResultSummary(total, start, end) {
    ...
    if (!total) {
      const emptyText = activeCount
        ? `No birds match your filters (${activeCount} active).`
        : hasDevBirdDataSource
          ? 'No bird records found.'
  ...
  function renderPage() {
    ...
    grid.innerHTML = '';
    if (!total) {
      grid.innerHTML = hasDevBirdDataSource
        ? '<div class="empty">No birds match your filters.</div>'
        : '<div class="empty">No bird records yet. Data source not connected yet.</div>';
      updatePager(0, 0, 0);
      updateBirdItemList([]);
      return;
    }
    ...
    pageSlice.forEach((plant) => {
      ...
      card.addEventListener('click', () => {
        ...
        const detailUrl = `${DETAIL_PATH_PREFIX}/${encodeURIComponent(plant.id)}?return=${encodeURIComponent(returnUrl)}`;
  ...
  function applyState(nextState, options = {}) {
    ...
    if (opts.trigger) {
      emitBirdCatalogFilterChange(opts.trigger);
    }
  }
  ...
  async function init() {
    localizeCatalogAnchors();
    setupImgObserver();
    try {
      const rawBirds = resolveBirdDataSource();
      plants = rawBirds
        .map((entry, idx) => normalizeBirdRecord(entry, idx))
        .filter(Boolean);
      plants.forEach((bird, idx) => {
        bird.__order = idx;
      });
      const indexPayload = resolveBirdIndexSource();
